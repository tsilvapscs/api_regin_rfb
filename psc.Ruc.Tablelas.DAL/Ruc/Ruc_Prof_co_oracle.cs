...
{
    public class Ruc_Prof_co_oracle 
    {
        // Variables ******************* 
        ...
        public void Update(OracleTransaction bd)
        {
            try
            {
                ...
                using (OracleCommand cmdToExecuteSql = new OracleCommand())
                {
                    ...
                    sSocio.AppendLine("and		trim(rpr_cgc_cpf_secd) = :v_rpr_cgc_cpf_secd");

                    cmdToExecuteSql.CommandText = sSocio.ToString();
                    cmdToExecuteSql.CommandType = CommandType.Text;

                    cmdToExecuteSql.Parameters.Add(new OracleParameter("v_rpr_rge_pra_protocolo", OracleType.VarChar, 0, ParameterDirection.Input, true, 0, 0, "", DataRowVersion.Proposed, _rpr_rge_pra_protocolo));
                    cmdToExecuteSql.Parameters.Add(new OracleParameter("v_rpr_cgc_cpf_secd", OracleType.VarChar, 0, ParameterDirection.Input, true, 0, 0, "", DataRowVersion.Proposed, _rpr_cgc_cpf_secd.Trim()));
                ...
                cmdToExecute.Parameters.Add(new OracleParameter("v_rpr_cgc_cpf_secd", OracleType.VarChar, 0, ParameterDirection.Input, true, 0, 0, "", DataRowVersion.Proposed, _rpr_cgc_cpf_secd.Trim()));
        ...
        public void UpdateContador(OracleTransaction bd)
        {
            try
            {
                StringBuilder Sql = new StringBuilder();
                //A diferença e que esta procedimento se tiver o registro ele atualiza
                ...
                using (OracleCommand cmdToExecuteSql = new OracleCommand())
                {
                    ...
                    if (int.Parse(pCount.ToString()) > 0)
                    {
                        using (OracleCommand cmdToExecuteRP = new OracleCommand())
                        {
                            Sql = new StringBuilder();

                            Sql.AppendLine(" update ruc_prof");
                            Sql.AppendLine(" Set    rpr_email = '" + _rpr_email + "'");
                            ...
                            cmdToExecuteRP.Parameters.Add(new OracleParameter("v_rpr_rge_pra_protocolo", OracleType.VarChar, 0, ParameterDirection.Input, true, 0, 0, "", DataRowVersion.Proposed, _rpr_rge_pra_protocolo));
                            cmdToExecuteRP.Parameters.Add(new OracleParameter("v_rpr_cgc_cpf_secd", OracleType.VarChar, 0, ParameterDirection.Input, true, 0, 0, "", DataRowVersion.Proposed, _rpr_cgc_cpf_secd.Trim()));


                            cmdToExecuteRP.CommandText = Sql.ToString();
                            cmdToExecuteRP.Transaction = bd;
                            cmdToExecuteRP.Connection = bd.Connection;
                            cmdToExecuteRP.ExecuteNonQuery();
                            return;
                        }
                    }
                }
                ...
                Sql.Append("	:v_rpr_tes_cod_estado, ");
                Sql.Append("	:v_rpr_zona_postal, ");
                Sql.Append("	evalnumeric(:v_rpr_tmu_cod_mun), ");
                Sql.Append("	:v_rpr_cgc_cpf_secd, ");
                Sql.Append("	:v_rpr_tge_ttip_pers, ");
                Sql.Append("	:v_rpr_tge_vtip_pers, ");
                Sql.Append("	:v_rpr_nomb, ");
